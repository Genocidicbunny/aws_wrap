										 ...
										 Awesomium::FocusedElementType focused_type)
{
	...
	/// on a page.
	void WebViewListener_View::OnAddConsoleMessage(	Awesomium::WebView* caller,
													...
													const Awesomium::WebString& source)
	{
		...
		if( cb != callbacks.end() )
		{
			cb->second.console(
				reinterpret_cast<cWebViewPtr_t>(caller),
				reinterpret_cast<const cWebStringPtr_t&>(message),
				line_number,
				reinterpret_cast<const cWebStringPtr_t&>(source)
			);
		}
	}
											   ...
											   const WebLoginDialogInfo& dialog_info)
{
  ...
  ///
void WebViewListener_Dialog::OnShowCertificateErrorDialog(	Awesomium::WebView* caller,
															bool is_overridable,
															const Awesomium::WebURL& url,
															Awesomium::CertError error) 
{
	...
	if( cb != callbacks.end() )
	{
		cWebCertificateDialogInfo_t minfo;

		minfo.page_url = reinterpret_cast<const cWebUrlPtr_t&>(url);
		minfo.cert_err = error;
		minfo.is_overridable = is_overridable;
  ...
  ///
void WebViewListener_Dialog::OnShowPageInfoDialog(			Awesomium::WebView* caller,
															const Awesomium::WebPageInfo& page_info) 
{
	auto cb = callbacks.find(caller);
	if( cb != callbacks.end() )
	{
		cWebPageInfDialogInfo_t minfo;
		minfo.page_url = reinterpret_cast<const cWebUrlPtr_t&>(page_info.page_url);
		minfo.content_status = page_info.content_status;
		minfo.security_status = page_info.security_status;
		minfo.cert_err = page_info.cert_error;
		minfo.cert_issuer = reinterpret_cast<const cWebStringPtr_t&>(page_info.cert_issuer);
		minfo.cert_subject = reinterpret_cast<const cWebStringPtr_t&>(page_info.cert_subject);
